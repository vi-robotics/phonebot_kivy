...
class FaceDetectionListener(PythonJavaClass):
    """Interface to get the face detection from Android Camera
    """
    __javainterfaces__ = ('android.hardware.Camera$FaceDetectionListener', )

    def __init__(self, callback):
        super(FaceDetectionListener, self).__init__()
        self._callback = callback

    @java_method('([Landroid/hardware/Camera$Face;Landroid/hardware/Camera;)V')
    def onFaceDetection(self, data, camera):
        self._callback(data, camera)


class CameraAndroid2(CameraBase):
    """
    ...
    def __init__(self, **kwargs):
        ...
        self._face_cb = FaceDetectionListener(self._on_face_detected)
        self._buflock = threading.Lock()
        self._face_detections = []
        super(CameraAndroid2, self).__init__(**kwargs)
    ...
    def _on_face_detected(self, data, camera):
        res = []
        for face in data:
            #print( ' == face == ')
            #print( 0.5 * (face.rect.top + face.rect.bottom))
            #print( 0.5 * (face.rect.left + face.rect.right))
            w = (face.rect.bottom - face.rect.top)
            h = (face.rect.right - face.rect.left)
            res.append([-face.rect.bottom, -face.rect.right, w, h])
        self._face_detections = res
    ...
    def start(self):
        super(CameraAndroid2, self).start()
        ...
        self._android_camera.setFaceDetectionListener(self._face_cb)

        self._android_camera.startPreview()
        self._android_camera.startFaceDetection()
        if self._update_ev is not None:
            self._update_ev.cancel()
        self._update_ev = Clock.schedule_interval(self._update, 1 / self.fps)

    def stop(self):
        super(CameraAndroid2, self).stop()
        if self._update_ev is not None:
            self._update_ev.cancel()
